  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Bridgetown Trading Co. Ltd"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Bridgetown Trading Co. Ltd" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest supplier-registration-expired-is-held-and-unoverridable
  (testing "expired Suppliers Register registration (3-year validity, Public Procurement Act 2021 s.86(5)) -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:supplier-registration-expired} (-> (store/ledger db) last :basis)))
...
(deftest tin-unverified-is-held-and-unoverridable
  (testing "unverified BRA Taxpayer Identification Number (TIN) when required -> HARD hold"
    (let [[db actor] (fresh)
