(ns marketentry.store
  "SSoT for the Barbados (BRB) market-entry compliance actor, behind a
...
(defn demo-data
  ...
  `:supplier-registration-date` / `:submission-date` are ground truth for
  the flagship date-recompute check (Public Procurement Act, 2021
  s.86(5), 3-year Suppliers Register validity); `:requires-tin?` /
  `:tin-verified?` are ground truth for the conditional BRA TIN check."
  []
  {:engagements
   {"eng-1" {:id "eng-1" :operator "Bridgetown Trading Co. Ltd" :portal "gov-bb.bonfirehub.com"
             ...
             :jurisdiction "BRB" :status :intake}
    "eng-2" {:id "eng-2" :operator "Atlantis LLC" :portal "gov-bb.bonfirehub.com"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :supplier-registration-date "2024-01-15" :submission-date "2026-07-22"
             :requires-tin? true :tin-verified? true
    ...
    "eng-3" {:id "eng-3" :operator "Holetown Builders Ltd" :portal "gov-bb.bonfirehub.com"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             ...
             :drafted? false :submitted? false
             :jurisdiction "BRB" :status :intake}
    "eng-4" {:id "eng-4" :operator "Speightstown Freight Ltd" :portal "gov-bb.bonfirehub.com"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             ...
             :drafted? false :submitted? false
             :jurisdiction "BRB" :status :intake}
    "eng-5" {:id "eng-5" :operator "Oistins Marine Services Ltd" :portal "gov-bb.bonfirehub.com"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :supplier-registration-date "2024-01-15" :submission-date "2026-07-22"
             ...
             :drafted? false :submitted? false
             :jurisdiction "BRB" :status :intake}
    "eng-6" {:id "eng-6" :operator "Warrens Civic Tech Ltd" :portal "gov-bb.bonfirehub.com"
             :base-fee 400000 :monthly-rate 25000 :monitoring-months 6
             :claimed-fee 550000.0
             :supplier-registration-date "2025-05-01" :submission-date "2026-07-22"
             :requires-tin? true :tin-verified? true
             :drafted? false :submitted? false
             :jurisdiction "BRB" :status :intake}}})
...
(defn- engagement->tx [{:keys [id operator portal base-fee monthly-rate monitoring-months claimed-fee
                               supplier-registration-date submission-date
  ...
  (cond-> {:engagement/id id}
    ...
    supplier-registration-date            (assoc :engagement/supplier-registration-date supplier-registration-date)
...
(def ^:private engagement-pull
  [:engagement/id :engagement/operator :engagement/portal :engagement/base-fee :engagement/monthly-rate
   ...
   :engagement/supplier-registration-date :engagement/submission-date
...
(defn- pull->engagement [m]
  (when (:engagement/id m)
    {:id (:engagement/id m) :operator (:engagement/operator m) :portal (:engagement/portal m)
     ...
     :supplier-registration-date (:engagement/supplier-registration-date m)
...
(defn kotobase-store
  ...
  [{:keys [db-api seq-fn]}]
  (->KotobaseStore (persist/store {:db-api db-api :actor "marketentry-brb"})
